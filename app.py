...
def webhook_ui():  
    ...
    # Eğer webhook URL'si zaten varsa, göster
    if webhooks:
        st.subheader("Mevcut Webhook URL")
        for webhook_name, webhook_info in webhooks.items():
            st.write(f"Webhook Adı: {webhook_name}, URL: {webhook_info['url']}")
    else:
        # Yeni Webhook ekle
        ...
        if st.button("Webhook'u Kaydet"):
            if webhook_url:
                # Webhook bilgilerini kaydet
                webhook_name = "VarsayılanWebhook"
                webhooks = {webhook_name: {"url": webhook_url}}  # Yeni URL'yi kaydet
                save_webhooks(webhooks)
                st.success(f"{webhook_name} adlı webhook başarıyla kaydedildi!")
            else:
                st.error("Lütfen Webhook URL'sini girin.")
    ...
    if st.session_state.audio_file and not st.session_state.recording:
        ...
        with st.spinner('Ses metne dönüştürülüyor...'):
            ...
            if transcript:
                ...
                current_time = datetime.now()
                meeting_date = current_time.strftime('%d.%m.%Y')  # Gün.Ay.Yıl
                meeting_time = current_time.strftime('%H:%M')

                st.markdown(f"""<div style="display: flex; justify-content: flex-end; color: #000000; font-weight: bold;">
                                Toplantı Tarihi: {meeting_date} <br> Toplantı Saati: {meeting_time}</div>""", unsafe_allow_html=True)
        ...
        # **Webhook gönderme**
        webhooks = load_webhooks()  # Kayıtlı webhookları yükle
        for webhook_name, webhook_info in webhooks.items():
            webhook_url = webhook_info['url']
            response_status = send_to_webhook(webhook_url, data)
            if response_status == 200:
                st.success(f"{webhook_name} webhookuna veri başarıyla gönderildi!")
            else:
                st.error(f"{webhook_name} webhookuna gönderme hatası: {response_status}")

                # Webhook yanıtını kontrol etmek için detaylı log ekleme
                try:
                    response_text = requests.post(webhook_url, json=data).text
                    st.warning(f"{webhook_name} webhook yanıtı: {response_text}")
                except Exception as e:
                    st.error(f"Webhook yanıtı alınamadı: {str(e)}")
